...
class PickAFile extends StatefulWidget {
  ...
  final String? initialFileAsByte;

  const PickAFile(
      {super.key, required this.onChanged, required this.initialFileAsByte});
...
class _PickAFileState extends State<PickAFile> {
  ...
  @override
  void initState() {
    fileBytes = widget.initialFileAsByte;
    super.initState();
  }
      ...
      children: [
        SizedBox(
            ...
            child: fileBytes == null
                ...
                : Image.memory(base64Decode(fileBytes!))),
        ElevatedButton(
            onPressed: () async {
              FilePickerResult? result = await FilePicker.platform.pickFiles(
                  allowMultiple: false,
                  type: FileType.custom,
                  allowedExtensions: ['jpg', 'png', 'jpeg', 'stl']);

              if (result != null) {
                setState(() {
                  fileBytes = base64Encode(
                      Uint8List.fromList(result.files.first.bytes!));
                });
                widget.onChanged(fileBytes);
